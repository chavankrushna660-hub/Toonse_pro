...
export class Engine {
  ...
  meshGrid: MeshGridType | null = null;
  meshDensityX = 10;
  meshDensityY = 10;
  meshPointSize = 20;
  meshShowGrid = true;
  meshShowPoints = true;
  meshPreviewMode = true;
  meshActivePointId: string | null = null;
  ...
  generateMeshGrid() {
    if (!this.selectedId || !this.objects[this.selectedId]) {
      this.meshGrid = null;
      return;
    }
  ...
  setMeshDensity(x: number, y: number) {
    this.meshDensityX = Math.max(2, Math.min(30, x));
    this.meshDensityY = Math.max(2, Math.min(30, y));
    if (this.tool === "deform") this.generateMeshGrid();
    this.notify();
    this.render();
  }

  setMeshPointSize(size: number) {
    this.meshPointSize = clamp(size, 10, 60);
    this.notify();
    this.render();
  }

  setMeshShowGrid(show: boolean) {
    this.meshShowGrid = show;
    this.notify();
    this.render();
  }

  setMeshShowPoints(show: boolean) {
    this.meshShowPoints = show;
    this.notify();
    this.render();
  }

  setMeshPreviewMode(preview: boolean) {
    this.meshPreviewMode = preview;
  ...
  handlePointerDown(e: React.PointerEvent<HTMLCanvasElement>) {
    ...
    if (this.tool === "deform" && this.selectedId) {
      ...
      if (bestPt) {
        ...
        this.dragStartLocal = { x: bestPt.currentX, y: bestPt.currentY };
  ...
  private drawDeformPoints(id: string) {
    if (!this.ctx || !this.objects[id] || !this.meshGrid) return;
    const ctx = this.ctx;
    ...
    if (this.meshShowPoints) {
      for (const pt of points) {
        ...
        if (!screen) continue;
        ctx.beginPath();
        if (pt.id === this.meshActivePointId) {
          ...
          ctx.arc(screen.x, screen.y, this.meshPointSize * 0.25, 0, Math.PI * 2);
        }
        ctx.fill();
